...
import { Authenticate, AuthResponse, User } from '../../models/user';
...
describe('AuthReducer', () => {
  ...
  describe('LOGIN_SUCCESS', () => {
    it('should add a user set loggedIn to true in auth state', () => {
      const user = { name: 'test_user', groups: [{ id: 1, name: 'test_group' }], email: 'test_email' } as User;
      const resp = { user: user, token: 'test_token' } as AuthResponse;
      const createAction = new LoginSuccess(resp);
